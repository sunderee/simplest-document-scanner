...
public class SimplestDocumentScannerPlugin: NSObject, FlutterPlugin {
    ...
    public func handle(
        ...
        case Self.scanMethod:
            do {
                let request = try DocumentScannerRequest(
                    arguments: call.arguments
                )
                nonisolated(unsafe) let flutterResult = result
                Task { @MainActor in
...
@MainActor
private final class VisionKitDocumentScanner: NSObject,
    @preconcurrency VNDocumentCameraViewControllerDelegate
{
    ...
    private func findTopmostViewController(from root: UIViewController?)
        -> UIViewController?
    {
    ...
    private func processScan(
        ...
        if request.enforceMaxPageLimit,
            ...
            for (index, image) in images.enumerated() {
                guard
                    let jpegData = image.jpegData(
                        compressionQuality: request.jpegQuality
                    )
                else {
    ...
    private func makePdfData(from images: [UIImage]) throws -> Data {
        ...
        let renderer = UIGraphicsPDFRenderer(
            bounds: CGRect(origin: .zero, size: firstSize)
        )
    ...
    var flutterError: FlutterError {
        ...
        case .noOutputFormats:
            return FlutterError(
                code: "INVALID_ARGUMENT",
                message:
                    "At least one of returnJpegs or returnPdf must be true.",
                details: nil
            )
        }
    }
